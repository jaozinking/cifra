...
export async function POST(req: Request) {
	try {
		const { productId, amount, description, customerEmail, userId, promoCode } =
			await req.json();
		...
		if (numAmount < 99) {
			return NextResponse.json({ error: 'Minimum amount is 99 RUB' }, { status: 400 });
		}

		// Get product to find owner
		const productResponse = await fetch(
			`${PB_URL}/api/collections/products/records/${productId}`,
			{
				cache: 'no-store',
			}
		);
		...
		if (promoCode) {
			// Валидация промокода перед использованием
			...
			try {
				// Безопасное использование промокода после валидации
				const safePromoCode = promoCode.toUpperCase().trim();
				const promo = await pb
					.collection('promos')
					.getFirstListItem(`code = "${safePromoCode}" AND isActive = true`);
				promoId = promo.id;
			} catch (_e) {
				console.warn(
					`Promo code ${promoCode} not found or inactive, proceeding without promo`
				);
			}
		}
