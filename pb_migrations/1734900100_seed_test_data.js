...
migrate(
		...
		const testProducts = [
			{
				...
				description:
					'Профессиональный шаблон Notion для управления проектами с автоматизацией, трекингом задач и аналитикой. Включает готовые базы данных, шаблоны страниц и интеграции.',
				priceRub: 1999,
			...
			},
			{
				...
				description:
					'Полный набор компонентов для создания современных интерфейсов. Включает кнопки, формы, карточки, навигацию и многое другое. Совместим с Figma Auto Layout.',
				priceRub: 2999,
			...
			},
			{
				...
				description:
					'Готовый React хук для работы с REST API. Включает обработку ошибок, кэширование, оптимистичные обновления и TypeScript типы.',
				priceRub: 1499,
			...
			},
			{
				...
				description:
					'Подробное руководство по монетизации SaaS продуктов. Стратегии ценообразования, метрики, кейсы успешных компаний и практические советы.',
				priceRub: 999,
			...
			},
			{
				...
				description:
					'Коллекция из 50 профессиональных звуковых эффектов для видео и подкастов. WAV формат, высокое качество, без роялти.',
				priceRub: 799,
	...
	(app) => {
		...
		try {
			const _usersCollection = app.findCollectionByNameOrId('users');
			const productsCollection = app.findCollectionByNameOrId('products');
			const salesCollection = app.findCollectionByNameOrId('sales');
			const promosCollection = app.findCollectionByNameOrId('promos');
			const payoutsCollection = app.findCollectionByNameOrId('payouts');
