...
const PublicStore: React.FC<PublicStoreProps> = ({ product, sellerSettings, onClose }) => {
	...
	const [processingState, setProcessingState] = useState<'idle' | 'processing' | 'success'>(
		'idle'
	);
	...
	const handleApplyPromo = async () => {
		...
		try {
			...
			if (found?.isActive) {
				...
				setPromoError('');
				toast.success(
					`Промокод "${found.code}" применен! Скидка ${found.discountPercent}%`
				);
			} else {
			...
			<div className="min-h-screen bg-black flex items-center justify-center p-4">
				<div className="max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-8 text-center animate-fade-in">
					...
					<p className="text-zinc-400 mb-8">
						Спасибо за покупку. Файлы отправлены на {email}
					</p>
	...
	return (
		<div className="bg-black min-h-screen flex flex-col relative">
			<div className="bg-violet-900/20 border-b border-violet-500/20 py-3 px-6 text-center text-xs font-medium text-violet-300">
				Режим предпросмотра.{' '}
				<span className="underline cursor-pointer" onClick={onClose}>
			...
			<div className="flex-1 flex flex-col items-center p-4 sm:p-8">
				<div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-12 mb-20">
						...
						<div className="flex items-center justify-between bg-zinc-900/50 p-4 rounded-xl border border-zinc-800/50 backdrop-blur-sm">
							<div className="flex items-center gap-4">
								<div className="w-12 h-12 rounded-full bg-zinc-800 overflow-hidden flex items-center justify-center shrink-0 relative">
									{sellerSettings.avatarUrl &&
									sellerSettings.avatarUrl.trim() !== '' ? (
										<Image
											src={sellerSettings.avatarUrl}
											alt={sellerSettings.displayName}
											fill
											className="object-cover"
											unoptimized={sellerSettings.avatarUrl.startsWith(
												'data:'
											)}
										/>
									) : (
										<span className="text-xl font-bold text-zinc-500">
											{sellerSettings.displayName.charAt(0)}
										</span>
									)}
								</div>
								<div>
					...
					<div
						className="text-zinc-100 space-y-8 animate-fade-in"
					...
					>
						...
						<div className="prose prose-invert prose-zinc max-w-none">
							{product.description.split('\n').map((line, i) => (
								<p
									key={`desc-line-${i}-${line.slice(0, 20)}`}
									className="mb-2 text-zinc-300 leading-relaxed"
								>
									{line}
								</p>
							))}
						...
						<div className="sticky bottom-4 z-10">
							<div className="bg-zinc-900/90 backdrop-blur-md rounded-xl border border-zinc-800 p-6 space-y-4 shadow-2xl">
								...
								<div className="flex justify-center gap-3 opacity-30 grayscale">
									<div className="h-6 w-10 bg-white rounded flex items-center justify-center text-[8px] text-black font-bold font-serif italic">
										VISA
									</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
							{otherProducts.map((other) => (
								...
								>
									...
									<div className="p-4">
										<h4 className="text-sm font-medium text-zinc-100 group-hover:text-violet-400 transition-colors line-clamp-1">
											{other.title}
										</h4>
			...
			<footer className="border-t border-zinc-900 bg-zinc-950/50 py-12 mt-auto">
				<div className="max-w-5xl mx-auto px-6 text-center">
					<div className="flex justify-center gap-8 text-xs text-zinc-500 mb-6">
						...
						<a
							href="mailto:support@cifra.store"
							className="cursor-pointer hover:text-zinc-300"
						>
							Поддержка платформы
						</a>
					...
					<p className="text-[10px] text-zinc-600">
						© 2024 Cifra. Платформа для цифровой дистрибуции. Безопасные платежи
							...
							{activeModal === 'privacy' ? (
								<>
									<p>
										...
										всей информации, которую Сайт может получить о Пользователе
										во время использования сайта, программ и продуктов сайта.
									</p>
									<p>
										1.1 В рамках настоящей Политики под «персональной
										информацией пользователя» понимаются: Личная информация,
										которую пользователь предоставляет о себе самостоятельно при
										регистрации (создании учётной записи) или в процессе
										использования Сервисов.
									</p>
									<p>
										1.2 Мы защищаем ваши данные с использованием современных
										стандартов шифрования и не передаем их третьим лицам, за
								...
								<>
									<p>
										Настоящий текст является публичной офертой (предложением) в
										соответствии со статьей 435 и пунктом 2 статьи 437
										Гражданского кодекса Российской Федерации.
									</p>
									<p>1. ПРЕДМЕТ ДОГОВОРА</p>
									<p>
										1.1. Продавец обязуется передать в собственность Покупателя
										Цифровой товар, а Покупатель обязуется принять и оплатить
										Товар на условиях настоящей Оферты.
									</p>
									<p>2. ВОЗВРАТ ТОВАРА</p>
									<p>
										2.1. Цифровые товары надлежащего качества не подлежат
										возврату или обмену на аналогичный товар. Будьте внимательны
										при покупке.
									</p>
								</>
							)}
					...
					<div className="relative w-full max-w-md bg-zinc-900 border border-zinc-800 sm:rounded-2xl rounded-t-2xl p-6 animate-fade-in shadow-2xl max-h-[90vh] overflow-y-auto">
						...
						{processingState === 'processing' ? (
							...
							<div className="space-y-4">
								<div className="bg-zinc-950 p-4 rounded-lg border border-zinc-800 flex flex-col gap-2">
									...
									{appliedPromo && (
										<div className="text-xs text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded w-fit">
											Скидка {appliedPromo.percent}% (Промокод:{' '}
											{appliedPromo.code})
										</div>
									)}
								</div>
								...
								{/* Email */}
								<div>
									<label className="block text-xs font-medium text-zinc-400 mb-1.5 uppercase">
										Email
									</label>
									<input
										type="email"
								...
								<div className="p-4 bg-zinc-950 border border-zinc-800 rounded-lg">
									...
									<p className="text-xs text-zinc-500">
										...
										защищенную страницу оплаты, где сможете выбрать способ
										оплаты: банковская карта, СБП, электронные кошельки и другие
										методы.
									</p>
								</div>
