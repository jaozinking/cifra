			...
			if (verifiedPayment.status === 'succeeded') {
				...
				try {
					const order = await pb
						.collection('orders')
						.getFirstListItem(`yookassa_payment_id="${paymentId}"`);
					...
					if (order && order.status === 'pending') {
						...
						const netAmount =
							Math.round((amount - platformFee - transactionFee) * 100) / 100;
						...
						if (order.promo) {
							try {
								...
								console.log(
									`✅ Promo code ${promo.code} usage count increased to ${(promo.uses || 0) + 1}`
								);
							} catch (promoError) {
								console.warn(
									'Failed to update promo code usage count:',
									promoError
								);
							}
						}
						...
						try {
							...
							if (customerEmailResult.success) {
								console.log(
									`✅ Purchase confirmation email sent to ${order.customerEmail} for order ${order.id}`
								);
							} else {
								console.error(
									`❌ Failed to send purchase confirmation email to ${order.customerEmail}:`,
							...
							try {
								...
								if (owner.emailNotifications !== false) {
									...
									if (sellerEmailResult.success) {
										console.log(
											`✅ Sale notification email sent to seller ${owner.email} for order ${order.id}`
										);
									} else {
										console.warn(
											`⚠️ Failed to send sale notification email to seller ${owner.email}:`,
											sellerEmailResult.error
										);
									}
								} else {
									console.log(
										`ℹ️ Email notifications disabled for seller ${owner.email}, skipping notification`
									);
								}
							} catch (emailError) {
							...
							console.error(
								'Failed to create download token or send email:',
								tokenError
							);
						}
					}
