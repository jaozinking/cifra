...
const Dashboard: React.FC<DashboardProps> = ({ onCreateClick, onEditClick, onProductClick }) => {
	...
	const handleDelete = async (e: React.MouseEvent, id: string) => {
		...
		if (
			confirm(
				'Вы уверены, что хотите удалить этот товар? Статистика продаж останется, но товар исчезнет из магазина.'
			)
		) {
			try {
	...
	const salesChartData = sales
		.reduce((acc: SalesDataPoint[], sale) => {
	...
	const onboardingProgress =
		([hasProducts, hasProfile, hasSales].filter(Boolean).length / 3) * 100;
		...
		<div className="space-y-8 animate-fade-in pb-12 relative">
				...
				<div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 mb-8">
					<div className="flex items-center justify-between mb-4">
						...
						<span className="text-sm font-mono text-violet-400">
							{Math.round(onboardingProgress)}% готово
						</span>
					</div>
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
						<div
							className={`flex items-center gap-3 p-3 rounded-lg border ${hasProducts ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-zinc-700 bg-zinc-800/30'}`}
						>
							{hasProducts ? (
							...
							<span className={hasProducts ? 'text-emerald-200' : 'text-zinc-400'}>
								Создать первый продукт
							</span>
						</div>
						<div
							className={`flex items-center gap-3 p-3 rounded-lg border ${hasProfile ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-zinc-700 bg-zinc-800/30'}`}
						>
							{hasProfile ? (
								<CheckCircle className="w-5 h-5 text-emerald-500" />
							) : (
							...
							<span className={hasProfile ? 'text-emerald-200' : 'text-zinc-400'}>
								Настроить аватар профиля
							</span>
						</div>
						<div
							className={`flex items-center gap-3 p-3 rounded-lg border ${hasSales ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-zinc-700 bg-zinc-800/30'}`}
						>
							{hasSales ? (
								<CheckCircle className="w-5 h-5 text-emerald-500" />
							) : (
							...
							<span className={hasSales ? 'text-emerald-200' : 'text-zinc-400'}>
								Получить первую продажу
							</span>
						</div>
					</div>
				</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
				...
				<div className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl shadow-sm">
					...
					<div className="text-2xl font-bold text-zinc-100">{totalSales}</div>
					<div className="mt-2 text-xs text-zinc-500">Цифровые копии</div>
				</div>
				...
				<div className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl shadow-sm">
					...
					<div className="mt-2 text-xs text-zinc-500">Просмотр в покупку</div>
				</div>
				...
				>
					...
					<button
						className={`mt-3 w-full py-1.5 text-xs font-medium rounded-lg transition-colors border ${availableBalance > 0 ? 'bg-emerald-600 hover:bg-emerald-500 text-white border-transparent' : 'bg-zinc-800 text-zinc-500 border-zinc-700 cursor-not-allowed'}`}
					>
						Вывести средства
					</button>
				</div>
						...
						>
							<div className="relative h-48 w-full bg-zinc-800 rounded-lg overflow-hidden">
								...
								<div className="absolute top-2 left-2">
									<span
										className={`px-2 py-1 rounded text-[10px] uppercase font-bold tracking-wider border backdrop-blur-sm ${product.status === 'published' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/20' : 'bg-zinc-900/80 text-zinc-400 border-zinc-700'}`}
									>
							...
							<div className="flex-1">
								...
								<h4 className="font-medium text-zinc-100 mt-1 group-hover:text-violet-400 transition-colors">
									{product.title}
								</h4>
					...
					{products.length === 0 && (
						<div className="col-span-full border-2 border-dashed border-zinc-800 rounded-xl p-12 text-center">
							...
							<p className="text-zinc-500 mb-6">
								Создайте первый цифровой продукт, чтобы начать зарабатывать
							</p>
							<button
								onClick={onCreateClick}
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4">
					...
					<div className="relative w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-2xl animate-fade-in">
						...
						<p className="text-sm text-zinc-400 mb-6">
							Доступно:{' '}
							<span className="text-emerald-400 font-bold">{availableBalance} ₽</span>
						</p>
						...
						<div className="space-y-4">
							...
							<div className="bg-yellow-900/10 border border-yellow-900/30 p-3 rounded-lg flex gap-3 items-start">
								...
								<p className="text-xs text-yellow-600/80">
									Средства поступят на счет в течение 1-3 рабочих дней (обычно
									моментально).
								</p>
							</div>
